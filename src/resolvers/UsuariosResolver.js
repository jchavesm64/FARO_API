...
const crearMensajeClave = (usuario, clave) => {
    return "<!DOCTYPE html>" +
        ...
        "<span style='display: block; text-align: center;'>Contraseña: <strong>" + clave + "</strong></span>" +
        "</div>" +
...
const crearMensajeCodigo = (usuario, codigo) => {
    return "<!DOCTYPE html>" +
        ...
        "<span style='display: block; text-align: center;'>Codigo de verificación: <strong>" + codigo + "</strong></span>" +
        "</div>" +
        "<footer style='background-color: #d1d0d0; width: 280px; padding:10px; margin-left: auto; margin-right: auto; border-radius: 0px 0px 25px 25px;'>" +
        "<span style='display: block; text-align: center;'>BETHEL SOLUCIONES TECNOLÓGICAS</span>" +
        "</footer>" +
...
export default {
    Query: {
        obtenerUsuariosActivos: async (_, { }) => {
            try {
                const usuarios = await Usuario.find({ estado: "ACTIVO" }).populate({ path: 'roles' });
        ...
        obtenerUsuarioAutenticado: async (root, args, { usuarioActual }) => {
            ...
            const usuario = await Usuario.findOne({ cedula: usuarioActual.cedula }).populate({ path: 'roles' });
            return {
                estado: true,
                data: usuario,
                message: ""
        ...
        obtenerUsuarioByCodigo: async (_, { codigo }) => {
            try {
                const usuarios = await Usuario.findOne({ cedula: codigo }).populate({ path: 'roles' });
                return usuarios;
            } catch (error) {
                return error;
            }
    ...
    Mutation: {
        ...
        recuperarClave: async (_, { codigo, nueva }) => {
            try {
                var clave_enc = await bcrypt.hash(nueva, 10);
                const result = await Usuario.findOneAndUpdate({ cedula: codigo }, { clave: clave_enc }, { new: true });
                if (result) {
                    return {
                        success: true,
                        message: "Contraseña cambiada correctamente"
